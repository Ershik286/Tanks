...
// Определяем типы callback-функций
...
void createOneLevel() {
    ...
    // Сначала пытаемся загрузить карту из DLL
    ...
    if (hDll) {
        ...
        LoadMapFunc LoadMapFromFile = reinterpret_cast<LoadMapFunc>(GetProcAddress(hDll, "LoadMapFromFile"));

        if (LoadMapFromFile) {
            // Создаем директорию если не существует
            CreateDirectoryW(wayDirectory.c_str(), NULL);

            // Пытаемся загрузить карту из файла через DLL
            ...
            if (success) {
                std::cout << "Карта успешно загружена из файла!" << std::endl;
                FreeLibrary(hDll);
                return; // Успешно загрузили - выходим
            }
            else {
                std::cout << "Файл карты не найден, будет создана новая карта..." << std::endl;
            }
        }
        FreeLibrary(hDll);
    }
    else {
        std::cout << "DLL не найдена, создаем карту вручную..." << std::endl;
    }

    // Если не удалось загрузить из DLL, создаем карту вручную
    std::cout << "Создание карты вручную..." << std::endl;
    ...
    for (int i = 0; i < HEIGHTBLOCK; i++) {
        for (int j = 0; j < WIDTHBLOCK; j++) {
            ...
            int randomHealth = rand() % 100;
            if (randomHealth < chanceHealth) {
    ...
    // Добавляем структуры на карту (ваш существующий код)
    for (int i = 4; i < 8; i++) {
        for (int j = 3; j < 5; j++) {
